...
Item {
    ...
    IpcHandler {
        ...
        function status(): string { return JSON.stringify({version: "0.5.0", pluginId: "seigliva.ha-watch", state: root.state, message: root.message, rules: root.config.rules.length, paused: root.paused}) }
